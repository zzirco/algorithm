...
public class Solution_D9_2382_미생물격리_서울_20반_신호준 {
	...
	// 미생물 정보 저장 클래스. 이차원 배열의 특정 인덱스를 라벨로 저장. 위치가 동일하다면 미생물 개수 내림차순 정렬.
	static class Microbe implements Comparable<Microbe> {
		...
		@Override
		// 현재 위치가 동일하다면 미생물 개수로 내림차순 정렬한다. 그렇지 않으면 라벨 오름차순 정렬
		public int compareTo(Microbe o) {
			return (this.label==o.label)?Integer.compare(o.cnt, this.cnt):Integer.compare(this.label, o.label);
		}
	}
